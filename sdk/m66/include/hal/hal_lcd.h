...
extern bool LCDIF_Initialize(void);
extern void LCDIF_WriteCommand(uint8_t Cmd);
extern void LCDIF_WriteData(uint8_t Data);
extern uint8_t LCDIF_ReadData(void);
extern bool LCDIF_AddCommandToQueue(uint32_t *CmdArray, uint32_t CmdCount, pRECT UpdateRect);
extern bool LCDIF_SetupLayer(TVLINDEX Layer, TPOINT Offset, uint32_t SizeX, uint32_t SizeY, TCFORMAT CFormat, uint8_t Alpha);
extern bool LCDIF_SetLayerEnabled(TVLINDEX Layer, bool Enabled, bool UpdateScreen);
