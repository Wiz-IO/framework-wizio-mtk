...
#define IRQ_DSP2CPU_CODE            0x1F // ??? i2c
...
extern void NVIC_Dummy(void); // load .S file with IRQ_Handler()

extern void NVIC_Initialize(void);
extern bool NVIC_RegisterIRQ(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, bool Enable);
extern bool NVIC_UnregisterIRQ(uint32_t SourceIdx);
extern bool NVIC_RegisterEINT(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, uint16_t Debounce, bool Enable);
extern bool NVIC_UnregisterEINT(uint32_t SourceIdx);
extern bool NVIC_EnableEINT(uint32_t SourceIdx);
extern bool NVIC_DisableEINT(uint32_t SourceIdx);
extern bool NVIC_EnableIRQ(uint32_t SourceIdx);
extern bool NVIC_DisableIRQ(uint32_t SourceIdx);
