...
static void GPT_InterruptHandler(void)
{
    ...
    do
    {
        if ((Status & GPT1_TO) && (GPTStatus.GPT.GPT1_Handler != NULL))
            GPTStatus.GPT.GPT1_Handler();
        if ((Status & GPT2_TO) && (GPTStatus.GPT.GPT2_Handler != NULL))
...
{
    if (!GPTStatus.GPT.GPTIntsRegistered)
        return true;
...
void GPT_InitializeTimers(void)
{
    ...
    if (!GPT_IsPoweredUp())
        GPT_PowerUp(); //Enable clock for GPT to initialize it
    GPTIMER1_CON = 0;  //Disable GPT1
    GPTIMER2_CON = 0;  //Disable GPT2
    if (!(GPTIMER4_CON & GPT4_LOCK))
        GPTIMER4_CON &= ~GPT4_Enable; //Disable GPT4 if it not locked
...
bool GPT_StartTimer(TGPT Index)
{
    ...
    case GP_TIMER4:
        if (!(GPTIMER4_CON & GPT4_LOCK))
            GPTIMER4_CON |= GPT4_Enable; //Try to enable timer
...
bool GPT_StopTimer(TGPT Index)
{
    ...
    case GP_TIMER4:
        if (!(GPTIMER4_CON & GPT4_LOCK))
            GPTIMER4_CON &= ~GPT4_Enable; //Try to disable timer
...
bool GPT_SetupTimer(TGPT Index, uint16_t Frequency, bool Arepeat, void (*Handler)(void), bool Start)
{
    ...
    case GP_TIMER1:
        ...
        if ((MAX_GPT_FREQ % Frequency) >= (Frequency >> 1))
            GPTValue++;
        ...
        if (!GPTStatus.GPT.GPTIntsRegistered && (Handler != NULL))
...
void GPT_SleepTimers(void)
{
    if (GPTStatus.GPTEnabled)
    {
        if (GPTStatus.GPT.GPT1_Enabled)
            GPTIMER1_CON &= ~GPT_Enable;
        if (GPTStatus.GPT.GPT2_Enabled)
            GPTIMER2_CON &= ~GPT_Enable;
        if (!(GPTIMER4_CON & GPT4_LOCK) && GPTStatus.GPT.GPT4_Enabled)
            GPTIMER4_CON &= ~GPT4_Enable;
        GPT_PowerDown();
#ifdef USEINTERRUPTS
        if (GPTStatus.GPT.GPTIntsRegistered)
            NVIC_DisableIRQ(IRQ_GPT_CODE);
#endif
    }
}
