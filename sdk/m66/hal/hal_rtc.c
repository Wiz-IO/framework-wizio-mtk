...
bool RTC_SetDate(pDATE Date)
{
    ...
    if (Date == NULL)
        return false;
    if (Date->Year > (RTC_INITIAL_YEAR + 127))
        return false;
...
bool RTC_SetAlarmDateTime(pDATETIME DateTime, bool UseInterrupt)
{
        ...
        (DateTime->Time.Sec < 60))
    {
        ...
        if (DateTime->Date.Year > (RTC_INITIAL_YEAR + 127))
            return false;
        if (!DateTime->Date.Month || (DateTime->Date.Month > 12))
            return false;
...
time_t RTC_getDateTime(void)
{
    ...
    ti.tm_sec = RTC_TC_SEC;
    ti.tm_min = RTC_TC_MIN;
    ti.tm_hour = RTC_TC_HOU;

    ti.tm_mday = RTC_TC_DOM;
    ti.tm_mon = RTC_TC_MTH;
    ti.tm_year = RTC_TC_YEA + RTC_INITIAL_YEAR - 1900;
