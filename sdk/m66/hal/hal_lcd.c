...
bool LCDIF_IsQueueRunning(void)
{
...
bool LCDIF_GetCommandFromQueue(void)
{
    pDLITEM tmpItem;
...
void LCDIF_RestartQueue(void)
{
    ...
    if (!LCDIF_IsQueueRunning() && LCDIF_GetCommandFromQueue())
        LCDIF_StartLCDTransfer();

    RestoreInterrupts(flags);
}

bool LCDIF_AddCommandToQueue(uint32_t *CmdArray, uint32_t CmdCount, pRECT UpdateRect)
{
    pLCDCMD CMD;
...
void LCDIF_ISR(void)
{
    ...
    {
        if (LCDIF_GetCommandFromQueue())
            LCDIF_START = LCDIF_RUN;
    }
    else
        LCD_DBG("Unsolicited LCDIF interrupt code 0x%04X!\r\n", IntID);
}

bool LCDIF_RegisterISR(void)
{
    return NVIC_RegisterIRQ(IRQ_LCD_CODE, LCDIF_ISR, IRQ_SENS_LEVEL, true);
}

bool LCDIF_UnregisterISR(void)
{
...
bool LCDIF_Initialize(void)
{
    ...
    if (LCDIFQueue == NULL)
        LCDIFQueue = DL_Create(0);
    if ((LCDIFQueue == NULL) || !LCDIF_RegisterISR())
    {
        if (LCDIFQueue == NULL)
        {
            LCD_DBG("Failed! (Insufficient memory)\r\n");
            LCD_DBG(" Total used %uKiB\r\n", GetTotalUsedMemory() >> 10);
        }
        else
            LCD_DBG("Failed! (Unable to register LCD ISR 0x%02X)\r\n", IRQ_LCD_CODE);
    ...
    LCDIF_DisableInterface();
    LCD_DBG("Failed! (Can't initialize LCD driver)\r\n");
    return false;
}

bool LCDIF_SetupLayer(TVLINDEX Layer, TPOINT Offset, uint32_t SizeX, uint32_t SizeY,
...
bool LCDIF_SetLayerEnabled(TVLINDEX Layer, bool Enabled, bool UpdateScreen)
{
    TRECT LayerRect;
