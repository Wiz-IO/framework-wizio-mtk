...
bool PMU_IsChargerDetected(void)
{
    return (CHR_CON0 & RGS_CHRDET) ? true : false;
}

bool PMU_IsPowerKeyPressed(void)
{
    return (STRUP_CON0 & QI_PWRKEY_DEB) ? false : true;
}

void PMU_SetVibrationOutput(bool Enable)
{
#ifdef VIBRVoltage
...
void PMU_SetISINKMode(bool UsePWM)
{
    if (UsePWM)
...
void PMU_SetISINKOutput(ISINKCHNL Channel, bool Enable)
{
...
void PMU_SetISINKParameters(ISINKCHNL Channel, ICVAL Value, bool Enable)
{
    volatile uint16_t *ISINK_CON;
    uint16_t En;
