...
bool USART_Initialize(TUSART *USART, uint32_t Speed, void (*Handler)(void), uint32_t Flags)
{
    USC_Pause_us(1000);
...
void USART_EnableInterrupts(TUSART *USART, TUSART_FLAGS Flags)
{
    if (USART != NULL)
    {
        ...
        if (Flags & USART_TXInt_EnFlag)
            USART->IER |= ETBEI; //Enable transmitter interrupt
        if (Flags & USART_RXInt_EnFlag)
            USART->IER |= ERBFI; //Enable receiver interrupt
        if (Flags & USART_ERRInt_EnFlag)
            USART->IER |= ELSI; //Enable errors interrupt
        USART->LCR = tmpLCR;
...
void USART_DisableInterrupts(TUSART *USART, TUSART_FLAGS Flags)
{
    if (USART != NULL)
    {
        ...
        if (Flags & USART_TXInt_EnFlag)
            USART->IER &= ~ETBEI; //Disable transmitter interrupt
        if (Flags & USART_RXInt_EnFlag)
            USART->IER &= ~ERBFI; //Disable receiver interrupt
        if (Flags & USART_ERRInt_EnFlag)
            USART->IER &= ~ELSI; //Disable errors interrupt
        USART->LCR = tmpLCR;
...
{
    if ((USART != NULL) && (USART->FCR_RD & FIFOEN))
        USART->FCR = CLRR | FIFOEN;
}

void USART_FlashTX(TUSART *USART)
{
    if ((USART != NULL) && (USART->FCR_RD & FIFOEN))
        USART->FCR = CLRT | FIFOEN;
}
...
void USART_SendBufferPoll(TUSART_CONTEXT *USARTINF)
{
    ...
    if ((USARTINF == NULL) || (USARTINF->USART == NULL))
        return;
