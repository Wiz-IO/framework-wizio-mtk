...
{
    if (SourceIdx < ADIE_NUM_IRQ_SOURCES)
        ADIE_IRQ_EOI2 = SourceIdx;
}

static void NVIC_SetEINT_EOI(uint32_t SourceIdx)
{
    if (SourceIdx < NUM_EINT_SOURCES)
        EINT_INTACK = (1 << SourceIdx);
}

static void NVIC_SetAEINT_EOI(uint32_t SourceIdx)
{
    if (SourceIdx < NUM_ADIE_EINT_SOURCES)
        ADIE_EINT_INTACK = (1 << SourceIdx);
}
...
static void NVIC_SetEINTDebounce(uint32_t SourceIdx, uint16_t Debounce)
{
    ...
    if (SourceIdx < NUM_EINT_SOURCES)
    {
        ...
        if (!Masked)
            NVIC_MaskEINT2(SourceIdx);                //1. Mask EINT
        ...
        if (!Masked)
            NVIC_UnmaskEINT2(SourceIdx); //5. Unmask EINT
    }
    else if (SourceIdx < GLB_EINT_SOURCES)
    {
        SourceIdx -= NUM_EINT_SOURCES;
        ...
        if (!Masked)
            NVIC_MaskEINT2(SourceIdx + NUM_EINT_SOURCES);                  //1. Mask EINT
        ...
        if (!Masked)
            NVIC_UnmaskEINT2(SourceIdx + NUM_EINT_SOURCES); //9. Unmask EINT
    }
}
    ...
    do
    {
        if (IRQSrcIdx != NOIRQ)
        {
            ...
            if (IRQHandlers[IRQSrcIdx].Handler != NULL)
            {
                ...
                //LOG("IRQSrcIdx = %ld", IRQSrcIdx);
            }
            else
            {
                //LOG("IRQ Unhandled 0x%02X", IRQSrcIdx);
            }
...
void NVIC_Initialize(void)
{
    ...
    while ((i = NVIC_GetIRQStatus2()) != NOIRQ)
        NVIC_SetIRQ_EOI(i); //Release IRQ interrupts
    while ((i = NVIC_GetAIRQStatus2()) != NOIRQ)
        NVIC_SetAIRQ_EOI(i); //Release AIRQ interrupts
...
bool NVIC_RegisterIRQ(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, bool Enable)
{
    ...
    if (SourceIdx < NUM_IRQ_SOURCES)
    {
        ...
        for (i = 0; i < MAX_IRQ_SELECTIONS; i++)
        {
            if ((FreeIdx == -1) && (IRQTable[i].SourceIdx == -1))
                FreeIdx = i;
        ...
        if (FreeIdx != -1)
        {
            ...
            if (Sense == IRQ_SENS_EDGE)
                NVIC_SetIRQSenseEdge(SourceIdx);
            else
                NVIC_SetIRQSenseLevel(SourceIdx);
    ...
    else if ((SourceIdx >= TOTAL_IRQ_SOURCES) && (SourceIdx < GLB_IRQ_SOURCES))
    {
        ...
        for (i = 0; i < MAX_ADIE_IRQ_SELECTIONS; i++)
        {
            if ((FreeIdx == -1) && (AIRQTable[i].SourceIdx == -1))
                FreeIdx = i;
...
bool NVIC_RegisterEINT(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, uint16_t Debounce, bool Enable)
{
    ...
    if (SourceIdx < NUM_EINT_SOURCES)
    {
        ...
        if (EINTHandlers[SourceIdx].Handler == Handler)
            return true;

        intflags = DisableInterrupts();
        EINTHandlers[SourceIdx].Handler = Handler;
        if (Sense == EINT_SENS_EDGE)
            NVIC_SetEINTSenseEdge(SourceIdx);
        else
            NVIC_SetEINTSenseLevel(SourceIdx);
    ...
    if (SourceIdx < GLB_EINT_SOURCES)
    {
        ...
        if (AEINTHandlers[SourceIdx - NUM_EINT_SOURCES].Handler == Handler)
            return true;
...
/*
    DEFAULT HANDLER
*/
void NVIC_C_Default_Handler(void)
{
    printf("!!! TRAP !!!");
    while (1)
        ;
