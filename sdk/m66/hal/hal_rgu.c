...
#include <hal/hal_rgu.h>

void RGU_SetWDTInterval(uint8_t Interval, bool Enable) //Interval in seconds
{
#define _MIN(a, b) ((a) < (b) ? (a) : (b))
    Interval = _MIN(Interval, RGUINTERVAL_MAX);
    WDT_MODE = WDMKEY;
    WDT_LENGTH = WDLKEY | TIMEOUT(Interval); //Set timeout value
    RGU_RestartWDT();
    if (Enable)
        WDT_MODE = WDMKEY | ENABLE; //Enable WDT
}
...
{
    if (WDT_STA & HW_WDT)
        return WDT_REASON_HW;
    else if (WDT_STA & SW_WDT)
        return WDT_REASON_SW;
